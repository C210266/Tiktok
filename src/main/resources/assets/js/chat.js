$(document).ready(function () {
  ...
  $array.forEach((data, idx) => {
    ...
    if (data.msgtype == "img") {
      $html += '<img src="/img/icons/ic_photo.svg" alt="read"/><p class="truncate text-xs text-gray-500">&nbsp;Photo</p>'
    }
    else if (data.status == "recording") {
      $html += '<img src="/img/icons/ic_recording.svg" alt="read"/><p class="truncate text-xs text-gray-500">&nbsp;0:14</p>'
    }
    else if (data.status == "typing") {
      $html += '<p class="text-xs text-gray-500">Typing......</p>'
    }
    else if (data.read == "true") {
      $html += '<img src="/img/icons/ic_read.svg" alt="read"/><p class="truncate text-xs text-gray-500">&nbsp; ' + data.msg + '</p>'
    }
    $html += `</div></div>
  ...
  $array.forEach((data, idx) => {
    if (data.type.split("|")[0] != "file") {
      if (data.sender == "me") {
        ...
        $html += `<div class="bg-blue-100 p-2 flex flex-col max-w-3/4 max-w-md mr-auto rounded-md relative">
          <div class="emoji transform translate-x-5 absolute right-0 top-1 cursor-pointer" data-index="`+ idx + `">
            <img src="img/icons/ic_emoticon.svg" alt="emoji"/>
          </div>
          ...
          <div id="dialog-`+ idx + `" class="dialog bg-black p-4 rounded-md absolute right-0 top-0 transform translate-x-1/2 -mr-3 -translate-y-16 flex gap-2 hidden">
            <img src="img/icons/ic_prev_emoji.svg" alt="select" />
            ...
            <img src="img/icons/ic_next_emoji.svg" alt="select" />
          </div>
        <p>`
      }
      ...
      $html += data.time + '</span>'
      if (data.read == "true") {
        $html += '<img src="/img/icons/ic_read.svg" alt="read" />'
      }
      $html += '</div></div>'
    }
    else {
      $html += `<div class="bg-gray-300 p-2 flex flex-col w-max max-w-md ml-auto rounded-md">
        <div class="flex justify-between p-2 rounded-md gap-4" style="background-color:#7676803d">
          <div class="flex gap-2 w-full items-center">
            <img src="/img/icons/ic_file.svg" alt="file" />
            <p class="whitespace-nowrap">FILE NAME</p>
          </div>
          <a href="#" class="cursor-pointer">
            <img src="/img/icons/ic_download.svg" alt="download" class />
          </a>
        </div>
        ...
        <p>PNG</p>
          ...
          <div class="flex ml-auto gap-2">
            ...
            <img src="/img/icons/ic_read.svg" alt="read" />
          </div>        
        </div>
      </div>`
    }
